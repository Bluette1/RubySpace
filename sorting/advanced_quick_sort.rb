...
#  Partition will need to use another trick to keep track of which
#  elements are greater and which are smaller than the pivot.
...
def partition(array, start, ending)
  ...
  while j <= ending - 1
    ...
    # ----------------------------------------------------------------------------------
    swap_posn = insert smaller_end_idx
    if array[j] < pivot

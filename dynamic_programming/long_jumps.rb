# Coding challenge:
# Given an array of positive integers. Get the maximum of an array, and that will be the maximum number of steps you can move left or right(i.e the length of your jump).Then get the value of the index and get the next landing. If that leads to the max posn, return the number of jumps.

# RightMove:
#  - Formular for calculating the next landing index, given the current index = (currIdx + n) % array.length
# LeftMove:
# - Invert the indexing from 0 - length - 1 so as to start from the last psition (length - 1) as 0. Apply the same formula
...
# But how do we know we have exhausted all the possibilities? If revisit the same node in a given path; we discard that path
...
def long_jumps(arr)
  ...
  if (max_idx == curr_idx_left)
    return jumps
  end

  if (max_idx == curr_idx_right)
    return jumps
  end

  if (max_idx <= curr_idx_left  + arr[curr_idx_left])
    return jumps + 1
  end

  if (max_idx <= curr_idx_right  + arr[curr_idx_right])
    return jumps + 1
  end
